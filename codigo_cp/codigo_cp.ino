...
void loop() {
  ...
  if (agora - tempoUltimaLeitura >= intervaloLeituraLDR) {
    ...
    luminosidade= map(luminosidade,0,1023,140,970);//funcao map 
    Serial.print("Luminosidade: ");
    Serial.println(luminosidade);
  }

  if (luminosidade >= lumALERT) { //estado de alerta(vermelho)
  ...
  } else if (luminosidade >= lumOK && luminosidade < lumALERT) { //estado de atenção (amarelo)
    digitalWrite(green, 0);
    ...
    digitalWrite(green, 1); //tudo okay!!!!!!
    digitalWrite(yellow, 0);
    digitalWrite(red, 0);
    alarme = false;
